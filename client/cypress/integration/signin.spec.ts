...
describe('드라이버 로그인 페이지 테스트', () => {
  ...
  it('회원가입 버튼을 클릭한다', () => {
    cy.get("[data-test-id='sign-up-btn']").click();
    cy.url().should('eq', `${Cypress.config().baseUrl}/driver/signup`);
  });

  it('로그인 버튼을 클릭한다', () => {
    cy.go('back');
    cy.get("[data-test-id='sign-in-btn']").click();
    cy.url().should('eq', `${Cypress.config().baseUrl}/driver/signin`);
  });

  it('잘못된 아이디 비밀번호를 입력하면 로그인에 실패한다.', () => {
    cy.get("[data-test-id='id-input']").clear().type(Cypress.env('driver_id'));
    cy.get("[data-test-id='pw-input']")
      .clear()
      .type(Cypress.env('driver_pw') + 's');
  ...
  it('올바른 아이디 비밀번호를 입력하면 로그인에 성공한다.', () => {
    cy.get("[data-test-id='id-input']").clear().type(Cypress.env('driver_id'));
    cy.get("[data-test-id='pw-input']").clear().type(Cypress.env('driver_pw'));
    cy.get('a').contains('로그인').click();
    cy.url().should('eq', `${Cypress.config().baseUrl}/driver/main`);
  });
});
